...
public class U_Mover : MonoBehaviour
{
    ...
    void Work_Move1(float dt)
    {
        ...
        RaycastHit SideHit = H_Rhit;
        float SideAng = Angls.z;
        if (!rgh)
        {
            SideAng = Angls.x;
            SideHit = H_Lhit;
        ...
        if (H_Chit.point.y > SideHit.point.y)
        {
            SideAng = Angls.y;
        }
        //if(Mathf.Abs(H_Chit.point.y - SideHit.point.y) < 0.05f)
        //{
        //    SideAng = Angls.y;
        //}
        Test.transform.eulerAngles = Vector3.forward * SideAng;

        Vector3 use = Vector3.zero;
